 function NPC_QUEST_242(cnum,reqNumber) -- 뮤네타(옵션 생성 아이템 NPC)
	local req = reqNumber

	if req == 1 then
		return 9,0,"Hei, kami dikenal sebagai Enchanter kembar! Kami bekerja keras tiap hari untuk memperkuat benda. Aku dapat menambahkan option padanya. Ingin tahu lebih lanjut?",2,"Chisel of Archer",3,"Chisel of Thief",4,"Chisel of Valor",5,"Chisel of Power",6,"Chisel of Rapidity",7,"Chisel of Accuracy",8,"Chisel of Fortune",9,"Chisel of Life"   
	...
	elseif req == 9 then
		return 3,0,"Chisel of Life meningkatkan 2 HP namun tidak 100% berhasil. Sebuah Amazonez diperlukan. Namun aku akan mengambil Core tersebut. Yakin ingin menukarnya?",82,"Ya, aku ingin menukarnya.",83,"Cara menggunakan Chisel of Life"
	...
	elseif req == 12 then
	  ...
	  if GetSwitchCount(cnum, 1026) > 0 then
		...
		if GetHench(cnum,1,81,1) > 1 then --잭오랜턴 81
		...
		AddHench(cnum,81,-1) --잭오랜턴 가져가기
		AddItemCount(cnum,311,1) -- 궁사의 정 추가
    ...
    if GetSwitchCount(cnum, 1027) > 0 then
			return 1,0,"Penukaran Chisel of Thief hanya tersedia sekali."
		end

		if GetHench(cnum,1,270,1) > 1 then 
			return 1,0,"Jika kamu memiliki lebih dari 2 Ballza dalam Inventory, kamu tidak dapat menukarkannya."
		end

		if GetHench(cnum,1,270,1) < 1 then
			return 1,0,"Kamu hanya perlu membawa Ballza, yang lainnya tidak perlu." 
		end   
		...
		AddHench(cnum,270,-1) --스캠퍼 가져가기
		AddItemCount(cnum,313,1) -- 도둑의 정 추가
	...
	elseif req == 32 then
	  ...
	  if GetSwitchCount(cnum, 1028) > 0 then
		...
		AddHench(cnum,109,-1) --라이노바 가져가기
		AddItemCount(cnum,312,1) -- 용기의 정 추가
	...
	elseif req == 42 then
	  ...
	  if GetSwitchCount(cnum, 1029) > 0 then
		...
		AddHench(cnum,37,-1) --그란샤 가져가기
		AddItemCount(cnum,314,1) -- 힘의 정 추가
	...
	elseif req == 52 then
	  ...
	  if GetSwitchCount(cnum, 1030) > 0 then
		...
		AddHench(cnum,173,-1) --터틀비틀 가져가기
		AddItemCount(cnum,315,1) -- 민첩의 정 추가
    ...
    if GetSwitchCount(cnum, 1031) > 0 then
		...
		AddHench(cnum,145,-1) --썬더버드 가져가기
		AddItemCount(cnum,316,1) -- 정확의 정 추가
    ...
    if GetSwitchCount(cnum, 1032) > 0 then
		...
		AddHench(cnum,242,-1) --지하걸 가져가기
		AddItemCount(cnum,317,1) -- 행운의 정 추가
    ...
    if GetSwitchCount(cnum, 1033) > 0 then
		...
		AddHench(cnum,196,-1) --킨타쿤테 가져가기
		AddItemCount(cnum,318,1) -- 생명의 정 추가
...
function NPC_QUEST_287(cnum,reqNumber) -- 제네타(업그레이드 아이템 NPC)
 local req = reqNumber
    ...
    if GetSwitchCount(cnum, 1034) > 0 then
		...
		AddHench(cnum,45,-1) --그린테일 가져가기
		AddItemCount(cnum,319,1) -- 티어스타 추가
